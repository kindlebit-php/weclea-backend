...
export const update_county = async(req,res)=>{
    ...
    try { 
    	...
    	for (var i = 0; i < city_ids.length; i++) {
			...
			dbConnection.query(qrySelect,[reqData.name, city, reqData.id], function (error, data) {
			if (error) throw error;
				if (data.length<=0) { 
					var addContnetQry = "update wc_county set `name`=?, city_id=?, state_id=? ,`status`=? where id=? ";
				    dbConnection.query(addContnetQry,[reqData.name, city_ids[k],reqData.state_id , 1,reqData.id], function (error, data) {
					if (error) throw error;
