...
export const submit_wash_detail = async (req, res) => {
  try {
    ...
    dbConnection.query(userIdQuery, [booking_id], function (error, data) {
      ...
      const customerId_Query = "SELECT u.customer_id,bins.delievery_instruction FROM users AS u JOIN booking_instructions AS bin ON u.id = bins.user_id WHERE u.id = ?";
